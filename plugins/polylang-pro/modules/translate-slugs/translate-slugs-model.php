...
class PLL_Translate_Slugs_Model {
	...
	public function __construct( &$polylang ) {
		...
		if ( $polylang instanceof PLL_Settings && isset( $_GET['tab'] ) && 'strings' == $_GET['tab'] ) {
	 ...
	 */
	public function remove_paged_from_link( $link ) {
		if ( isset( $this->translated_slugs['paged'] ) ) {
			$slugs = $this->translated_slugs['paged']['translations'];
			$slugs[] = $this->translated_slugs['paged']['slug'];
			...
			foreach ( $this->model->get_languages_list() as $language ) {
				...
				foreach ( get_post_types() as $type ) {
					...
					if ( ! empty( $type->rewrite['slug'] ) && $this->model->is_translated_post_type( $type->name ) ) {
						$slug = trim( $type->rewrite['slug'], '/' ); // It seems that some plugins add / (ex: woocommerce)
						$slugs[ $type->name ]['slug'] = $slug;
						$slugs[ $type->name ]['translations'][ $language->slug ] = $mo->translate( $slug );
						...
						if ( ! empty( $type->has_archive ) ) {
							...
							$slugs[ 'archive_' . $type->name ]['translations'][ $language->slug ] = $mo->translate( $slug );
						}
					}
				}
				...
				foreach ( get_taxonomies() as $tax ) {
					...
					if ( ! empty( $tax->rewrite['slug'] ) && ( $this->model->is_translated_taxonomy( $tax->name ) || 'post_format' == $tax->name ) ) {
						$slugs[ $tax->name ]['slug'] = $tax->rewrite['slug'];
						$slugs[ $tax->name ]['translations'][ $language->slug ] = $mo->translate( $tax->rewrite['slug'] );
					}
				}
				...
				foreach ( array( 'author', 'search', 'attachment' ) as $slug ) {
					...
					$slugs[ $slug ]['translations'][ $language->slug ] = $mo->translate( $slug );
				}

				$slugs['paged']['slug'] = 'page';
				$slugs['paged']['translations'][ $language->slug ] = $mo->translate( 'page' );

				if ( ! empty( $wp_rewrite->front ) ) {
					$slug = trim( $wp_rewrite->front, '/' );
					$slugs['front']['slug'] = $slug;
					$slugs['front']['translations'][ $language->slug ] = $mo->translate( $slug );
				}
	...
	public function prepare_rewrite_rules( $pre ) {
		if ( ! has_filter( 'rewrite_rules_array', array( $this, 'rewrite_translated_slug' ) ) ) {
			$this->init_translated_slugs();
	...
	protected function translate_post_type_archive_rule( $rules ) {
		...
		foreach ( $rules as $key => $rule ) {
			if ( $cpts && preg_match( $cpts, $rule, $matches ) && ! strpos( $rule, 'name=' ) ) {
				$post_type = $matches[1];

				if ( isset( $this->translated_slugs[ 'archive_' . $post_type ] ) ) {
	...
	protected function translate_paged_rule( $rules ) {
		...
		foreach ( $rules as $key => $rule ) {
			if ( strpos( $key, '/page/' ) && $count = preg_match_all( '#\[\d\]#', $rule ) ) {
				$newrules[ str_replace( '/' . $old, '/' . $new, $key ) ] = str_replace(
					$this->range( $count, 8 ),
					$this->range( $count + 1, 9 ),
					$rule
				); // Hopefully it is sufficient!
			} elseif ( 0 === strpos( $key, 'page/' ) && $count = preg_match_all( '#\[\d\]#', $rule ) ) {

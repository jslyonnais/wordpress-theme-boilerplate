...
class PLL_Frontend_Share_Term_Slug extends PLL_Share_Term_Slug {
	...
	public function get_term( $term, $taxonomy ) {
		...
		if ( $this->model->is_translated_taxonomy( $taxonomy ) ) {
			...
			// Since WP 4.4 get_term_by calls get_term so we have one function more in the stack
			$n = version_compare( $GLOBALS['wp_version'], '4.4alpha', '<' ) ? 3 : 4;
		...
		// Backward compatibility with WP < 4.4
		if ( version_compare( $GLOBALS['wp_version'], '4.4alpha', '<' ) ) {
			return $term;
		}
